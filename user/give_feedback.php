...
//$sql=mysqli_query($conn,"select * from faculty where semester='".$users['semester']."'");
  $sql=mysqli_query($conn,"select courses.*,faculty.* from faculty inner join courses on faculty.id = courses.t_id and  courses.semister='".$users['semister']."'");
	while($r=mysqli_fetch_array($sql))
	{

	echo "<option value='".$r['Name']."'>".$r['course_title']."</option>";
	}
		 ?>
</select>
</td>
